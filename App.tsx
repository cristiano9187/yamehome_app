...
function App() {
  ...
  // LOGIQUE CRUCIALE : On vérifie l'URL immédiatement au démarrage
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    if (!idToLoad) return;
    setIsSaving(true);
    // On force l'état visuel immédiatement
  ...
  // --- EFFET DÉMARRAGE : CHARGEMENT AUTO ---
  useEffect(() => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    if (isReadOnly) return; // Sécurité absolue
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans text-xs">
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => window.location.href = window.location.origin + window.location.pathname} className="bg-gray-700 px-2 py-1 rounded font-bold">QUITTER</button>
            <button onClick={() => { setFormData(getInitialState()); setIsReadOnly(false); setSearchId(''); }} className="bg-red-600 px-2 py-1 rounded font-bold">NOUVEAU</button>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            ...
            <select disabled={isReadOnly} name="apartmentName" value={formData.apartmentName} className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 outline-none disabled:opacity-50" onChange={handleChange}>
            ...
            {TARIFS[formData.apartmentName]?.units && TARIFS[formData.apartmentName].units!.length > 1 && (
              <div className="mb-3 p-2 bg-blue-900/30 border border-blue-500 rounded">
                <select disabled={isReadOnly} name="calendarSlug" value={formData.calendarSlug} onChange={handleChange} className="w-full bg-gray-700 p-1.5 rounded border border-blue-400 disabled:opacity-50">
          ...
          {/* TARIFICATION */}
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-gray-400">Paiements</h3>
            ...
            {formData.isCustomRate && (
              <input 
                disabled={isReadOnly} 
                type="number" 
                name="customLodgingTotal" 
                value={formData.customLodgingTotal || ''} // Correction du "0" : on affiche vide si c'est 0
                className="w-full bg-gray-700 rounded p-2 border border-yellow-600 text-yellow-300 mb-3 outline-none disabled:opacity-50" 
                placeholder="Montant total plateforme" 
            ...
            {formData.isNegotiatedRate && (
              <input 
                disabled={isReadOnly} 
                type="number" 
                name="negotiatedPricePerNight" 
                value={formData.negotiatedPricePerNight || ''} 
                className="w-full bg-gray-700 rounded p-2 border border-blue-500 text-blue-300 mb-3 outline-none disabled:opacity-50" 
                placeholder="Prix nuit négocié" 
                onChange={handleChange} 
              />
            )}
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2">
                ...
                {!isReadOnly && <button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px] font-bold shadow">+ ADD</button>}
              </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            <select disabled={isReadOnly} name="hosts" multiple value={formData.hosts} onChange={handleChange} className="w-full bg-gray-700 rounded p-2 text-[10px] h-20 mb-3 border border-gray-600 outline-none disabled:opacity-50 text-gray-300">
              {HOSTS.map(h => <option key={h.id} value={h.label}>{h.label}</option>)}
            </select>
            <input disabled={isReadOnly} type="text" name="signature" value={formData.signature} placeholder="Signature" className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 outline-none disabled:opacity-50 shadow-inner" onChange={handleChange} />
            <textarea disabled={isReadOnly} name="observations" value={formData.observations} rows={2} placeholder="Observations..." className="w-full bg-gray-700 rounded p-2 border border-gray-600 text-[10px] outline-none disabled:opacity-50" onChange={handleChange}></textarea>
          </div>
        </form>
      </div>
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          ...
          <div className="flex gap-2">
            {!isReadOnly && (
              <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-600'} text-white font-bold py-2 px-3 rounded shadow-md uppercase transition-all`}>
                {isSaving ? '...' : saveStatus === 'success' ? 'OK' : 'SAUVEGARDER'}
              </button>
            )}
            <button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded shadow-md uppercase">PDF</button>
          </div>
        </div>
