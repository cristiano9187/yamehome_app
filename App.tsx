...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    ...
    try {
      ...
      if (data.error) alert("Reçu non trouvé");
        ...
        setFormData({ ...data, hosts: data.hosts || [] });
  ...
  const saveToSheets = async () => {
    ...
    // --- FIX COLORATION : On détecte le Slug automatiquement ---
    const units = TARIFS[formData.apartmentName]?.units || [];
    const finalSlug = units.length === 1 ? units[0] : formData.calendarSlug;
    if (units.length > 1 && !finalSlug) return alert("Précisez l'unité spécifique (Menu bleu)");
      ...
      <div className="min-h-screen bg-gray-900 text-white p-6 font-sans flex flex-col items-center">
        <div className="w-full max-w-md bg-gray-800 p-6 rounded-lg border border-blue-500/30">
          ...
          <div className="space-y-4">
            ...
            <button onClick={submitCleaningReport} disabled={isSaving || !cleaningReport.agent} className="w-full bg-blue-600 font-bold py-4 rounded-lg mt-4 uppercase shadow-lg">{isSaving ? 'ENVOI...' : 'VALIDER'}</button>
            <button onClick={() => setIsCleaningMode(false)} className="w-full text-gray-400 text-xs mt-2">Annuler et retour</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 p-8 rounded-lg shadow-2xl border border-gray-700 w-full max-w-sm text-center">
          ...
          <input type="password" placeholder="Pass" className="w-full bg-gray-700 text-white rounded p-3 mb-4 border border-gray-600 outline-none" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} autoFocus />
          <button type="submit" className="w-full bg-blue-600 font-bold py-3 rounded uppercase">Entrer</button>
        </form>
      </div>
    );
  }
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl border-r border-gray-800">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => window.location.href = window.location.origin + window.location.pathname} className="bg-gray-700 px-2 py-1 rounded font-bold uppercase transition-all">Quitter</button>
            <button onClick={() => { setFormData(getInitialState()); setIsReadOnly(false); setSearchId(''); }} className="bg-red-600 px-2 py-1 rounded font-bold uppercase shadow-lg">Nouveau</button>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-gray-400 text-center text-[10px]">Client</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            ...
            <div className="flex gap-4 mb-3">
              <label className="flex items-center text-[10px]"><input disabled={isReadOnly} type="checkbox" name="isCustomRate" checked={formData.isCustomRate} onChange={handleChange} className="mr-1" /> Plateforme</label>
            ...
            {formData.isCustomRate && <input disabled={isReadOnly} type="number" name="customLodgingTotal" value={formData.customLodgingTotal || ''} className="w-full bg-gray-700 rounded p-2 border border-yellow-600 text-yellow-300 mb-3 outline-none" placeholder="Montant total plateforme" onChange={handleChange} />}
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2"><span className="font-bold text-gray-400 uppercase font-mono text-[10px]">Versements</span>{!isReadOnly && <button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase">+ Add</button>}</div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            ...
            <input disabled={isReadOnly} type="text" name="signature" value={formData.signature} placeholder="Signature" className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 outline-none disabled:opacity-50" onChange={handleChange} />
            <textarea disabled={isReadOnly} name="observations" value={formData.observations} rows={2} placeholder="Note..." className="w-full bg-gray-700 rounded p-2 border border-gray-600 outline-none disabled:opacity-50 text-[10px]" onChange={handleChange}></textarea>
          </div>
        </form>
      </div>
          ...
          <div className="flex gap-2">
            {!isReadOnly && (
              <>
                ...
                <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-600'} text-white font-bold py-2 px-3 rounded shadow uppercase text-[10px]`}>
