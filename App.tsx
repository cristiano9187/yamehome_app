...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    if (!idToLoad) return;
    setIsSaving(true);
    // On verrouille IMMÉDIATEMENT si on vient du calendrier
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans text-xs">
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            ...
            {TARIFS[formData.apartmentName]?.units && TARIFS[formData.apartmentName].units!.length > 1 && (
              <div className="mb-3 p-2 bg-blue-900/30 border border-blue-500 rounded">
                <label className="text-[10px] font-bold block mb-1 uppercase">Unité Physique</label>
          ...
          {/* VERSEMENTS */}
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-gray-400">Versements</h3>
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2"><span className="font-bold text-gray-400 uppercase font-mono">Détail</span><button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase shadow">+ Add</button></div>
              {formData.payments.map((p) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 text-[10px] relative">
                   <button onClick={() => setFormData(prev => ({...prev, payments: prev.payments.length > 1 ? prev.payments.filter(x => x.id !== p.id) : prev.payments}))} className="absolute top-1 right-1 text-red-400 font-bold px-1 z-10 text-[11px]">✕</button>
                   ...
                   <div className="flex gap-2">
                    ...
                    <select value={p.method} onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, method: e.target.value} : x)}))} className="bg-gray-800 rounded p-1 flex-1 outline-none border-none shadow-inner">{PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                   </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg text-[10px]">
            <select name="hosts" multiple value={formData.hosts} onChange={handleChange} className="w-full bg-gray-700 rounded p-2 h-24 mb-3 border border-gray-600 shadow-inner">
            ...
            <textarea name="observations" value={formData.observations} rows={2} placeholder="Observations..." className="w-full bg-gray-700 rounded p-2 border border-gray-600 outline-none" onChange={handleChange}></textarea>
          </div>
        </form>
      </div>
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          ...
          <div className="flex gap-2">
            {!isReadOnly && (
              <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-500'} text-white font-bold py-2 px-3 rounded shadow uppercase transition-all shadow-md`}>
                {isSaving ? '...' : saveStatus === 'success' ? 'OK' : 'SAUVEGARDER'}
              </button>
            )}
            <button onClick={() => window.print()} className="bg-blue-600 text-white font-bold py-2 px-3 rounded shadow uppercase shadow-md">PDF</button>
          </div>
        </div>
