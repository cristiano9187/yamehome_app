...
function App() {
  ...
  // --- NOUVEAU : ANNULATION LOGIQUE (SOFT DELETE) ---
  const softDeleteBooking = async () => {
    if (window.confirm("❗ ANNULATION : Voulez-vous retirer cette réservation du calendrier ? Les données resteront en base mais seront marquées 'ANNULE'.")) {
      setIsSaving(true);
      try {
