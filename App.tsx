...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string) => {
    ...
    try {
      ...
      else { setFormData(data); setIsReadOnly(true); }
  ...
  useEffect(() => {
    ...
    if (idFromUrl) { setSearchId(idFromUrl); loadReceipt(idFromUrl); }
  }, [loadReceipt]);
  ...
  const saveToSheets = async () => {
    ...
    // LOGIQUE DE SECURITE POUR LE CALENDRIER
    ...
    if (units.length > 1 && !finalSlug) {
      return alert("Veuillez choisir l'unité spécifique (Studio ou Chambre)");
    }
      ...
      <div className="w-full md:w-1/3 text-white p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setFormData(getInitialState()); setIsReadOnly(false); }} className="bg-red-500 text-[9px] px-2 py-1 rounded font-bold">NOUVEAU</button>
          </div>
        </div>
        ...
        <div className="bg-blue-900/20 p-4 rounded border border-blue-500/30 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => loadReceipt(searchId)} className="bg-blue-600 text-white text-[10px] px-3 py-2 rounded font-bold uppercase shadow-lg">OK</button>
          </div>
        </div>
        ...
        <form className="space-y-4 text-sm" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2"><span className="text-[10px] font-bold text-gray-400 uppercase">Historique</span><button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px]">+ ADD</button></div>
              {formData.payments.map((p) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 text-[10px]">
                   ...
                   <div className="flex gap-2">
                    ...
                    <select value={p.method} onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, method: e.target.value} : x)}))} className="bg-gray-800 rounded p-1 flex-1">{PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                   </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            ...
            <input type="text" name="signature" value={formData.signature} placeholder="Signature" className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 text-xs shadow-inner" onChange={handleChange} />
