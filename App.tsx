...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    if (!idToLoad) return;
    setIsSaving(true);
    // On verrouille tout de suite si c'est un chargement auto
    ...
    try {
      ...
      if (data.error) alert("Introuvable");
      else {
        setFormData(data);
      }
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans">
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden">
        ...
        <div className="bg-blue-900/20 p-4 rounded border border-blue-500/30 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => loadReceipt(searchId, false)} className="bg-blue-600 text-[10px] px-3 py-2 rounded font-bold">OK</button>
          </div>
        </div>
        ...
        <form className="space-y-4 text-sm" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            ...
            <input type="text" name="signature" value={formData.signature} placeholder="Signature" className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 text-xs shadow-inner outline-none" onChange={handleChange} />
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          <div className="flex flex-col"><h2 className="text-gray-600 font-bold text-sm">APERÇU REÇU</h2><span className="text-[10px] text-gray-400 font-mono font-bold">{formData.receiptId}</span></div>
          <div className="flex gap-2">
            {!isReadOnly && (
            ...
            <button onClick={() => window.print()} className="bg-blue-600 text-white font-bold py-2 px-3 rounded text-[10px] shadow">PDF</button>
          </div>
        </div>
