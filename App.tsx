...
const ACCESS_PASSWORD = "odza2026"; // Change ce mot de passe par celui de ton choix
...
function App() {
  ...
  useEffect(() => {
    const authStatus = localStorage.getItem('yame_auth');
    if (authStatus === 'true') {
      setIsAuthenticated(true);
    }
  }, []);

  // --- LOGIQUE TITRE DYNAMIQUE ---
  useEffect(() => {
    if (!isAuthenticated) {
      document.title = "Accès Restreint - YameHome";
      return;
    }
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (passwordInput === ACCESS_PASSWORD) {
      setIsAuthenticated(true);
      localStorage.setItem('yame_auth', 'true'); // On s'en souvient
    } else {
  ...
  const handleLogout = () => {
    localStorage.removeItem('yame_auth');
    setIsAuthenticated(false);
  };

  // --- ACTIONS FORMULAIRE (Inchangées) ---
  const resetForm = () => {
    if (window.confirm("Créer un nouveau reçu ?")) {
  ...
  // ... (addPayment, updatePayment, removePayment, handleChange restent identiques)
  const addPayment = () => {
  ...
  const removePayment = (id: string) => { if (formData.payments.length > 1) setFormData(prev => ({ ...prev, payments: prev.payments.filter(p => p.id !== id) })); };
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value, type } = e.target;
    if (type === 'checkbox') setFormData(prev => ({ ...prev, [name]: (e.target as HTMLInputElement).checked }));
    else if (name === 'hosts') setFormData(prev => ({ ...prev, hosts: Array.from((e.target as HTMLSelectElement).selectedOptions).map(opt => opt.text) }));
    else setFormData(prev => ({ ...prev, [name]: type === 'number' ? parseFloat(value) || 0 : value }));
  };

  // --- RENDU 1 : ÉCRAN DE CONNEXION ---
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 w-full max-w-md text-center">
          <h1 className="text-3xl font-bold text-blue-400 mb-2">YameHome</h1>
          <p className="text-gray-400 text-sm mb-8 font-semibold uppercase tracking-widest">Générateur de Reçus</p>
          
          <div className="mb-6">
            <label className="block text-left text-xs font-bold text-gray-500 uppercase mb-2 ml-1">Code d'accès requis</label>
            <input 
              type="password" 
              placeholder="Entrez le mot de passe"
              className="w-full bg-gray-700 rounded-lg p-4 text-white border border-gray-600 focus:border-blue-500 outline-none text-center text-xl tracking-widest"
              value={passwordInput}
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-lg shadow-lg transition-all">
            DÉVERROUILLER
          </button>
        </form>
      </div>
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 font-sans">
      ...
      <div className="w-full md:w-1/3 text-white p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl relative">
        ...
        <div className="mb-6 flex justify-between items-center">
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-green-500 font-bold uppercase tracking-widest">Connecté</span>
          </div>
          <div className="flex gap-2">
            <button onClick={resetForm} className="bg-red-500 hover:bg-red-600 text-white text-[9px] px-2 py-1 rounded font-bold uppercase">Nouveau</button>
            <button onClick={handleLogout} className="bg-gray-700 hover:bg-gray-600 text-white text-[9px] px-2 py-1 rounded font-bold uppercase">Quitter</button>
          </div>
        </div>
        ...
        <form className="space-y-4 text-sm" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h3 className="text-gray-400 uppercase text-xs font-bold mb-3 tracking-wider">Client</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h3 className="text-gray-400 uppercase text-xs font-bold mb-3 tracking-wider">Réservation</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h3 className="text-gray-400 uppercase text-xs font-bold mb-3 tracking-wider">Versements</h3>
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Détails</span>
              ...
              {formData.payments.map((p, idx) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 relative">
                  {formData.payments.length > 1 && <button onClick={() => removePayment(p.id)} className="absolute top-1 right-1 text-red-400 text-[10px]">✕</button>}
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h3 className="text-gray-400 uppercase text-xs font-bold mb-3 tracking-wider">Signature & Note</h3>
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          ...
          <div className="flex gap-2">
            <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-500'} text-white font-bold py-2 px-3 rounded text-[10px] transition-all shadow`}>
