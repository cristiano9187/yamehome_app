...
function App() {
  ...
  // --- CHARGEMENT INITIAL (REÇU OU MÉNAGE) ---
  ...
  const submitCleaningReport = async () => {
    if (!cleaningReport.agent) return alert("Veuillez saisir le nom de l'agent");
    setIsSaving(true);
    const isManual = !urlParams.get('menageId');
    const payload = {
          ...
          <div className="mb-6 text-left border-b border-gray-700 pb-4">
            {isManual ? (
               <div className="space-y-2">
                 <select className="w-full bg-gray-700 rounded p-2 text-xs" name="manualApt" value={cleaningReport.manualApt} onChange={handleCleaningChange}>
          ...
          <div className="space-y-4 text-left">
            ...
            {isCleaningReadOnly ? (
               <div className="flex flex-col gap-2 mt-4">
                 ...
                 <button onClick={() => window.location.href = window.location.origin + window.location.pathname} className="w-full text-gray-400 text-center text-xs">Retour au formulaire Reçu</button>
               </div>
            ) : (
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans text-xs">
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl border-r border-gray-800">
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-sm">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-gray-400 text-[10px]">Logement</h3>
      ...
      {/* APERÇU DROITE */}
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container text-white">
