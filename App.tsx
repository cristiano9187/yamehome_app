...
function App() {
  ...
  const getInitialState = (): ReceiptData => ({
    ...
    signature: '', 
    hosts: [], 
    electricityCharge: false, packEco: false, observations: ''
  });
  ...
  // --- LOGIQUE TITRE ET PERSISTANCE ---
  useEffect(() => {
    if (!isReadOnly && !isCleaningMode) localStorage.setItem('yame_draft', JSON.stringify(formData));
  }, [formData, isReadOnly, isCleaningMode]);
  ...
  const saveToSheets = async () => {
    ...
    try {
      ...
      localStorage.removeItem('yame_draft');
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 p-8 rounded-lg shadow-2xl border border-gray-700 w-full max-w-sm text-center font-sans">
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded uppercase transition-all">Entrer</button>
        </form>
      </div>
    );
  }
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl border-r border-gray-800">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { localStorage.removeItem('yame_draft'); setFormData(getInitialState()); setIsReadOnly(false); setSearchId(''); }} className="bg-red-600 hover:bg-red-700 px-2 py-1 rounded font-bold uppercase transition-all shadow-lg">Nouveau</button>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            ...
            <textarea disabled={isReadOnly} name="observations" value={formData.observations} rows={2} placeholder="Note..." className="w-full bg-gray-700 rounded p-2 border border-gray-600 text-[10px] outline-none shadow-inner disabled:opacity-50" onChange={handleChange}></textarea>
          </div>
        </form>
      </div>
