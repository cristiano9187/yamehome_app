...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    ...
    try {
      ...
      const data = await response.json(); // FIX : Utilise bien 'response'
      if (data.error) {
  ...
  // --- 3. GESTION DES CHANGEMENTS (FIX EXCLUSIVITÉ + "0") ---
  ...
  const submitCleaningReport = async () => {
    ...
    try {
      await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify(payload) });
      alert("Envoyé !"); window.location.href = window.location.origin + window.location.pathname;
    } catch (e) { alert("Erreur"); } finally { setIsSaving(false); }
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans text-xs">
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl border-r border-gray-800">
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg text-[10px]">
            <select disabled={isReadOnly} name="hosts" multiple value={formData.hosts || []} onChange={handleChange} className="w-full bg-gray-700 rounded p-2 h-20 mb-3 border border-gray-600">
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          <div className="flex flex-col"><h2 className="text-gray-600 font-bold text-sm uppercase font-mono">Aperçu</h2><span className="text-[10px] text-gray-400 font-mono font-bold uppercase">{formData.receiptId}</span></div>
          <div className="flex gap-2">
            {!isReadOnly && (
              <>
