...
function App() {
  ...
  // --- LOGIQUE TITRE DYNAMIQUE (Pour nom du PDF) ---
  useEffect(() => {
  ...
  // --- GESTION DES CHANGEMENTS (AVEC EXCLUSIVITÉ TARIFS) ---
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      // LOGIQUE D'EXCLUSIVITÉ : Plateforme vs Négocié
      if (name === 'isCustomRate') {
        setFormData(prev => ({ 
          ...prev, 
          isCustomRate: isChecked, 
          isNegotiatedRate: isChecked ? false : prev.isNegotiatedRate 
        }));
      } else if (name === 'isNegotiatedRate') {
        setFormData(prev => ({ 
          ...prev, 
          isNegotiatedRate: isChecked, 
          isCustomRate: isChecked ? false : prev.isCustomRate 
        }));
      } else {
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setFormData(getInitialState()); setIsReadOnly(false); setSearchId(''); }} className="bg-red-600 text-[9px] px-2 py-1 rounded font-bold uppercase transition-all shadow-lg">Nouveau</button>
          </div>
        </div>
        ...
        <div className="bg-blue-900/20 p-4 rounded border border-blue-500/30 mb-6 text-center shadow-inner">
           ...
           <div className="flex gap-2">
            ...
            <button onClick={() => loadReceipt(searchId, false)} className="bg-blue-600 px-3 py-2 rounded font-bold uppercase hover:bg-blue-700 transition-all">OK</button>
           </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            ...
            <input disabled={isReadOnly} type="tel" name="phone" value={formData.phone} placeholder="Tél" className="w-full bg-gray-700 rounded p-2 border border-gray-600 text-xs" onChange={handleChange} />
          </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-sm">
            ...
            {formData.isCustomRate && <input disabled={isReadOnly} type="number" name="customLodgingTotal" value={formData.customLodgingTotal || ''} className="w-full bg-gray-700 rounded p-2 border border-yellow-600 text-yellow-300 mb-3 text-xs outline-none" placeholder="Montant plateforme" onChange={handleChange} />}
            {formData.isNegotiatedRate && <input disabled={isReadOnly} type="number" name="negotiatedPricePerNight" value={formData.negotiatedPricePerNight || ''} className="w-full bg-gray-700 rounded p-2 border border-blue-500 text-blue-300 mb-3 text-xs outline-none" placeholder="Prix nuit négocié" onChange={handleChange} />}
              ...
              {formData.payments.map((p) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 text-[10px] relative">
                   ...
                   <div className="flex gap-2">
                    ...
                    <select disabled={isReadOnly} value={p.method} onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, method: e.target.value} : x)}))} className="bg-gray-800 rounded p-1 flex-1 outline-none border-none shadow-inner">{PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                   </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            ...
            <textarea disabled={isReadOnly} name="observations" value={formData.observations} rows={2} placeholder="Note..." className="w-full bg-gray-700 rounded p-2 border border-gray-600 text-[10px] outline-none shadow-inner" onChange={handleChange}></textarea>
          </div>
        </form>
      </div>
