...
function App() {
  ...
  // --- CHARGEMENT AUTOMATIQUE VIA URL (?id=RC-123456) ---
  useEffect(() => {
  ...
  const resetForm = () => { if (window.confirm("Vider le formulaire ?")) setFormData(getInitialState()); };

  const saveToSheets = async () => {
    if (!formData.apartmentName || !formData.lastName) return alert("Remplir Nom et Appartement");
    
    // Déterminer l'unité calendrier finale
    ...
    if (possibleUnits.length > 1 && !finalSlug) {
        return alert("Veuillez choisir l'unité spécifique pour le calendrier (Matera/Bangangté)");
    }
    ...
    const payload = {
      ...
      calendarSlug: finalSlug, // On envoie l'ID précis pour le calendrier
  ...
  const loadReceipt = async () => {
    ...
    try {
      ...
      if (data.error) alert("Reçu non trouvé"); else setFormData(data);
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900">
      ...
      <div className="w-full md:w-1/3 text-white p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          <button onClick={resetForm} className="bg-red-500 text-white text-[9px] px-2 py-1 rounded font-bold uppercase">Nouveau Client</button>
        </div>

        <div className="bg-blue-900/20 p-4 rounded border border-blue-500/50 mb-6">
          <label className="text-blue-300 text-[10px] font-bold uppercase block mb-2">Recharger un reçu</label>
        ...
        <form className="space-y-4 text-sm" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="text-gray-400 uppercase text-[10px] font-bold mb-3 border-b border-gray-700 pb-1">Client</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="text-gray-400 uppercase text-[10px] font-bold mb-3 border-b border-gray-700 pb-1">Réservation</h3>
            ...
            {/* CHOIX UNITÉ SI MULTIPLE */}
            {TARIFS[formData.apartmentName]?.units && TARIFS[formData.apartmentName].units!.length > 1 && (
              <div className="mb-3 p-2 bg-blue-900/30 border border-blue-500 rounded animate-pulse">
                <label className="text-[10px] text-blue-300 font-bold block mb-1">UNITÉ PHYSIQUE CALENDRIER</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col"><label className="text-[9px] text-gray-500">Arrivée</label><input type="date" name="startDate" value={formData.startDate} className="bg-gray-700 rounded p-2 border border-gray-600 text-[11px]" onChange={handleChange} /></div>
              <div className="flex flex-col"><label className="text-[9px] text-gray-500">Départ</label><input type="date" name="endDate" value={formData.endDate} className="bg-gray-700 rounded p-2 border border-gray-600 text-[11px]" onChange={handleChange} /></div>
            </div>
          </div>

          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="text-gray-400 uppercase text-[10px] font-bold mb-3 border-b border-gray-700 pb-1">Versements</h3>
            ...
            {formData.isCustomRate && <input type="number" name="customLodgingTotal" value={formData.customLodgingTotal} className="w-full bg-gray-700 rounded p-2 border border-yellow-600 text-yellow-300 mb-3 text-xs" placeholder="Total total" onChange={handleChange} />}
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2"><span className="text-[10px] font-bold text-gray-400 uppercase">Historique</span><button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px]">+ AJOUTER</button></div>
              {formData.payments.map((p) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 relative">
                   <input type="date" value={p.date} onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, date: e.target.value} : x)}))} className="bg-gray-800 rounded p-1 mb-1 w-full text-[10px]" />
                   <div className="flex gap-2">
                    <input type="number" value={p.amount || ''} placeholder="Montant" onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, amount: parseFloat(e.target.value) || 0} : x)}))} className="bg-gray-800 rounded p-1 flex-1 font-bold text-green-400 text-[10px]" />
                    <select value={p.method} onChange={(e) => setFormData(prev => ({...prev, payments: prev.payments.map(x => x.id === p.id ? {...x, method: e.target.value} : x)}))} className="bg-gray-800 rounded p-1 flex-1 text-[10px] border-none">{PAYMENT_METHODS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                   </div>
                </div>
              ))}
            </div>
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          <div className="flex flex-col"><h2 className="text-gray-600 font-bold text-sm">Aperçu direct</h2><span className="text-[10px] text-gray-400 font-mono font-bold uppercase">{formData.receiptId}</span></div>
          <div className="flex gap-2">
            <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-500'} text-white font-bold py-2 px-3 rounded text-[10px] shadow uppercase`}>{isSaving ? '⏳' : saveStatus === 'success' ? 'OK' : 'Sauver'}</button>
