...
function App() {
  ...
  const loadReceipt = useCallback(async (idToLoad: string, setReadOnly: boolean) => {
    if (!idToLoad) return;
    setIsSaving(true);
    // On verrouille le mode lecture tout de suite pour la sécurité
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-900 text-white font-sans text-[11px]">
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setFormData(getInitialState()); setIsReadOnly(false); setSearchId(''); }} className="bg-red-500 px-2 py-1 rounded font-bold uppercase">Nouveau</button>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <input type="text" name="firstName" value={formData.firstName} placeholder="Prénom" className="w-full bg-gray-700 rounded p-2 border border-gray-600 outline-none" onChange={handleChange} />
              <input type="text" name="lastName" value={formData.lastName} placeholder="Nom" className="w-full bg-gray-700 rounded p-2 border border-gray-600 outline-none" onChange={handleChange} />
            </div>
            <input type="tel" name="phone" value={formData.phone} placeholder="Téléphone" className="w-full bg-gray-700 rounded p-2 border border-gray-600 outline-none" onChange={handleChange} />
          </div>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            ...
            <div className="mt-4 border-t border-gray-700 pt-3">
              <div className="flex justify-between items-center mb-2"><span className="font-bold text-gray-400 uppercase">Historique</span><button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded font-bold uppercase">+ Add</button></div>
              {formData.payments.map((p) => (
                <div key={p.id} className="bg-gray-700/40 p-2 rounded mb-2 border border-gray-600 relative">
                   <button onClick={() => setFormData(prev => ({...prev, payments: prev.payments.length > 1 ? prev.payments.filter(x => x.id !== p.id) : prev.payments}))} className="absolute top-1 right-1 text-red-400 font-bold px-1 z-10 text-[12px]">✕</button>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-lg">
            ...
            <input type="text" name="signature" value={formData.signature} placeholder="Signature" className="w-full bg-gray-700 rounded p-2 border border-gray-600 mb-3 outline-none shadow-inner" onChange={handleChange} />
