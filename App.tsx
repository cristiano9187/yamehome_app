...
function App() {
  ...
  // --- CHARGEMENT DES DONNÉES (REÇU OU MÉNAGE) ---
  const loadData = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`${SCRIPT_URL}?id=${id || mId}`);
      ...
      if (!data.error) {
        if (id) {
          ...
          // On remplit le rapport de ménage avec les données trouvées
          setCleaningReport({
            agent: data.agent || '',
          ...
          setIsCleaningReadOnly(true); // Bloqué en consultation par défaut
        }
      }
  ...
  const submitCleaningReport = async () => {
    if (!cleaningReport.agent) return alert("Nom agent requis");
    setIsSaving(true);
    const payload = {
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-6 font-sans flex flex-col items-center">
        <div className="w-full max-w-md bg-gray-800 p-6 rounded-lg border border-blue-500/30 shadow-2xl">
          ...
          <p className="text-[11px] text-gray-400 mb-6 uppercase font-bold border-b border-gray-700 pb-2">
            {urlParams.get('slug')} — {urlParams.get('date')}
          </p>
          
          <div className="space-y-4 text-left">
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase">Agent</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase">Statut</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 font-bold uppercase">Feedback</label>
            ...
            {isCleaningReadOnly ? (
               <div className="flex flex-col gap-2 mt-4">
                 ...
                 <button onClick={() => window.location.href = window.location.origin + window.location.pathname} className="w-full text-gray-400 text-center text-xs">Retour</button>
               </div>
            ) : (
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 p-8 rounded-lg shadow-2xl border border-gray-700 w-full max-w-sm text-center font-sans text-white">
          ...
          <button type="submit" className="w-full bg-blue-600 font-bold py-3 rounded uppercase transition-all shadow-lg">Entrer</button>
        </form>
      </div>
    );
  }
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-auto md:h-screen print:hidden shadow-2xl border-r border-gray-800">
        ...
        <div className="bg-blue-900/20 p-4 rounded border border-blue-500/30 mb-6 text-center">
          <div className="flex gap-2">
            ...
            <button onClick={() => loadReceipt(searchId)} className="bg-blue-600 px-3 py-2 rounded font-bold uppercase transition-all text-[10px]">OK</button>
          </div>
        </div>
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-md">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-[10px]">Client</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-sm">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-[10px]">Logement</h3>
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700 shadow-sm">
            <h3 className="uppercase font-bold mb-3 border-b border-gray-700 pb-1 italic text-center text-[10px]">Tarification</h3>
            ...
            <div className="mt-4 border-t border-gray-700 pt-3 text-[10px]">
              <div className="flex justify-between items-center mb-2 font-bold uppercase"><span>Versements</span>{!isReadOnly && <button type="button" onClick={() => setFormData(prev => ({...prev, payments: [...prev.payments, { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], amount: 0, method: 'Espèces' }]}))} className="bg-blue-600 text-white px-2 py-0.5 rounded shadow">+ Add</button>}</div>
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-2 md:p-8 flex flex-col items-start md:items-center overflow-y-auto h-auto md:h-screen preview-container">
        <div className="mb-4 no-print flex w-full max-w-[210mm] justify-between items-center print:hidden px-2">
          ...
          <div className="flex gap-2 text-[10px] font-bold">
            {!isReadOnly && (
              <>
                <button onClick={softDeleteBooking} className="bg-red-600 text-white py-2 px-3 rounded shadow-md uppercase transition-all">Annuler</button>
                <button onClick={saveToSheets} disabled={isSaving} className={`${saveStatus === 'success' ? 'bg-green-600' : 'bg-orange-600'} text-white py-2 px-3 rounded shadow uppercase transition-all`}>
                  {isSaving ? '...' : saveStatus === 'success' ? 'OK' : 'SAUVEGARDER'}
                </button>
              </>
            )}
            <button onClick={() => window.print()} className="bg-blue-600 text-white py-2 px-3 rounded shadow uppercase">PDF</button>
          </div>
        </div>
